...
function showProduct() {
    ...
    httpReq.addEventListener("readystatechange", function() {
        ...
        var Products = JSON.parse(httpReq.response).recipes;
        var Product ;
        for (let i = 0; i < Products.length; i++) {
                Product += 
                `<div class="w-25 ">
                    <div class="pro h-100">
                        <img src="${Products[i].image_url}" class="w-100 h-50" alt="${Products[i].recipe_id}" />
                        <h3 class="pt-3">${Products[i].title}</h3>
                        <p>${Products[i].publisher}</p>
                    </div>
                </div>`}
                document.querySelector(".row").innerHTML = Product;
}})}